// TEST FILE for StatsCalculator class
 ...
 *
 * The first command line argument, available as argv[1] is expected to be the
 * path of an input file containing a white-space separated list of numeric values.
 *
 * The second command line argument, available as argv[2] is expected to be the
 ...
 * in order to:
 *
 * 1) Open the input file, parse its contents and store the numeric values in an
 * internal std::vector<double>.
 * 2) Print a statistical summary comprising the sum, mean and standard deviation
 * of the numeric values to the terminal.
 * 3) Individually access each statistic and print their values to the terminal with
 * annotation.
 * 4) Write a statistical summary comprising the sum, mean and standard deviation
 * of the numeric values to the user-specified output file.
 *
 * The program returns zero on success and 1 if an incorrect number of command line
